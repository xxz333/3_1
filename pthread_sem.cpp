#include<iostream>
#include<pthread.h>
#include<sys/time.h>
#include <semaphore.h>
#include <xmmintrin.h> //SSE
#include <emmintrin.h> //SSE2
#include <pmmintrin.h> //SSE3
#include <tmmintrin.h> //SSSE3
#include <smmintrin.h> //SSE4.1
#include <nmmintrin.h> //SSSE4.2
#include <immintrin.h> //AVX、AVX2
using namespace std;

const int worker_count=8;
int n;
float** A;
struct timeval val;
struct timeval newval;

//线性数据结构定义
typedef struct
{
    int t_id;
}threadParam_t;

//信号量定义
sem_t sem_leader;
sem_t sem_Division[worker_count-1];
sem_t sem_Elimination[worker_count-1];

//线程函数定义
void *threadFunc(void *param)
{
    threadParam_t *p=(threadParam_t*)param;
    int t_id=p->t_id;
    for(int k=0;k<n;k++)
    {
        if(t_id==0)//主线程进行除法操作
        {
            for(int j=k+1;j<n;j++)
                A[k][j]=A[k][j]/A[k][k];
            A[k][k]=1.0;
        }
        else
            sem_wait(&sem_Division[t_id-1]);//阻塞，等待完成除法操作
        
        //t_id为0的线程唤醒其他工作线程，进行消去操作
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Division[i]);
        }

        //循环划分任务
        for(int i=k+1+t_id;i<n;i+=worker_count)
        {
            for(int j=k+1;j<n;j++)
                A[i][j]=A[i][j]-A[i][k]*A[k][j];
            A[i][k]=0.0;
        }
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_wait(&sem_leader);//等待其他worker完成消去
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Elimination[i]);//通知其他worker进入下一轮
        }
        else
        {
            sem_post(&sem_leader);//通知leader，已完成消去任务
            sem_wait(&sem_Elimination[t_id-1]);//等待通知，进入下一轮
        }
    }
    pthread_exit(NULL);
}

//线程函数定义(sse并行化不对齐)
void *threadFunc1(void *param)
{
    threadParam_t *p=(threadParam_t*)param;
    int t_id=p->t_id;
    for(int k=0;k<n;k++)
    {
        if(t_id==0)//主线程进行除法操作
        {
            __m128 vt=_mm_set1_ps(A[k][k]);
            int j;
            for(j=k+1;j+4<=n;j+=4)
            {
                __m128 va=_mm_loadu_ps(&A[k][j]);
                va=_mm_div_ps(va,vt);
                _mm_storeu_ps(&A[k][j],va);
            }
            for(;j<n;j++)
                A[k][j]=A[k][j]/A[k][k];
            A[k][k]=1.0;
        }
        else
            sem_wait(&sem_Division[t_id-1]);//阻塞，等待完成除法操作
        
        //t_id为0的线程唤醒其他工作线程，进行消去操作
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Division[i]);
        }

        //循环划分任务
        for(int i=k+1+t_id;i<n;i+=worker_count)
        {
            __m128 vaik=_mm_set1_ps(A[i][k]);
            int j;
            for(j=k+1;j+4<=n;j+=4)
            {
                __m128 vakj=_mm_loadu_ps(&A[k][j]);
                __m128 vaij=_mm_loadu_ps(&A[i][j]);
                //vx ← vakj*vaik;
                __m128 vx=_mm_set1_ps(0.0);
                vx=_mm_mul_ps(vakj,vaij);
                vaij=_mm_sub_ps(vaij,vx);
                //store4FloatTo(&A[i,j],vaij);
                _mm_storeu_ps(&A[i][j],vaij);
            }
            for(;j<n;j++)
                A[i][j]=A[i][j]-A[i][k]*A[k][j];
            A[i][k]=0.0;
        }
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_wait(&sem_leader);//等待其他worker完成消去
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Elimination[i]);//通知其他worker进入下一轮
        }
        else
        {
            sem_post(&sem_leader);//通知leader，已完成消去任务
            sem_wait(&sem_Elimination[t_id-1]);//等待通知，进入下一轮
        }
    }
    pthread_exit(NULL);
}

//线程函数定义(avx并行化不对齐)
void *threadFunc2(void *param)
{
    threadParam_t *p=(threadParam_t*)param;
    int t_id=p->t_id;
    for(int k=0;k<n;k++)
    {
        if(t_id==0)//主线程进行除法操作
        {
            __m256 vt=_mm256_set1_ps(A[k][k]);
            int j;
            for(j=k+1;j+8<=n;j+=8)
            {
                __m256 va=_mm256_loadu_ps(&A[k][j]);
                va=_mm256_div_ps(va,vt);
                _mm256_storeu_ps(&A[k][j],va);
            }
            for(;j<n;j++)
                A[k][j]=A[k][j]/A[k][k];
            A[k][k]=1.0;
        }
        else
            sem_wait(&sem_Division[t_id-1]);//阻塞，等待完成除法操作
        
        //t_id为0的线程唤醒其他工作线程，进行消去操作
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Division[i]);
        }

        //循环划分任务
        for(int i=k+1+t_id;i<n;i+=worker_count)
        {
            __m256 vaik=_mm256_set1_ps(A[i][k]);
            int j;
            for(j=k+1;j+8<=n;j+=8)
            {
                __m256 vakj=_mm256_loadu_ps(&A[k][j]);
                __m256 vaij=_mm256_loadu_ps(&A[i][j]);
                //vx ← vakj*vaik;
                __m256 vx=_mm256_set1_ps(0.0);
                vx=_mm256_mul_ps(vakj,vaij);
                vaij=_mm256_sub_ps(vaij,vx);
                //store4FloatTo(&A[i,j],vaij);
                _mm256_storeu_ps(&A[i][j],vaij);
            }
            for(;j<n;j++)
                A[i][j]=A[i][j]-A[i][k]*A[k][j];
            A[i][k]=0.0;
        }
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_wait(&sem_leader);//等待其他worker完成消去
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Elimination[i]);//通知其他worker进入下一轮
        }
        else
        {
            sem_post(&sem_leader);//通知leader，已完成消去任务
            sem_wait(&sem_Elimination[t_id-1]);//等待通知，进入下一轮
        }
    }
    pthread_exit(NULL);
}

//线程函数定义(avx512并行化不对齐)
void *threadFunc3(void *param)
{
    threadParam_t *p=(threadParam_t*)param;
    int t_id=p->t_id;
    for(int k=0;k<n;k++)
    {
        if(t_id==0)//主线程进行除法操作
        {
            __m512 vt=_mm512_set1_ps(A[k][k]);
            int j;
            for(j=k+1;j+16<=n;j+=16)
            {
                __m512 va=_mm512_loadu_ps(&A[k][j]);
                va=_mm512_div_ps(va,vt);
                _mm512_storeu_ps(&A[k][j],va);
            }
            for(;j<n;j++)
                A[k][j]=A[k][j]/A[k][k];
            A[k][k]=1.0;
        }
        else
            sem_wait(&sem_Division[t_id-1]);//阻塞，等待完成除法操作
        
        //t_id为0的线程唤醒其他工作线程，进行消去操作
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Division[i]);
        }

        //循环划分任务
        for(int i=k+1+t_id;i<n;i+=worker_count)
        {
            __m512 vaik=_mm512_set1_ps(A[i][k]);
            int j;
            for(j=k+1;j+16<=n;j+=16)
            {
                __m512 vakj=_mm512_loadu_ps(&A[k][j]);
                __m512 vaij=_mm512_loadu_ps(&A[i][j]);
                //vx ← vakj*vaik;
                __m512 vx=_mm512_set1_ps(0.0);
                vx=_mm512_mul_ps(vakj,vaij);
                vaij=_mm512_sub_ps(vaij,vx);
                //store4FloatTo(&A[i,j],vaij);
                _mm512_storeu_ps(&A[i][j],vaij);
            }
            for(;j<n;j++)
                A[i][j]=A[i][j]-A[i][k]*A[k][j];
            A[i][k]=0.0;
        }
        if(t_id==0)
        {
            for(int i=0;i<worker_count-1;i++)
                sem_wait(&sem_leader);//等待其他worker完成消去
            for(int i=0;i<worker_count-1;i++)
                sem_post(&sem_Elimination[i]);//通知其他worker进入下一轮
        }
        else
        {
            sem_post(&sem_leader);//通知leader，已完成消去任务
            sem_wait(&sem_Elimination[t_id-1]);//等待通知，进入下一轮
        }
    }
    pthread_exit(NULL);
}

void m_reset()
{
	for(int i=0;i<n;i++){
		for(int j=0;j<i;j++)
			A[i][j]=0;
		A[i][i]=1.0;
		for(int j=i+1;j<n;j++)
			A[i][j]=rand()%100;
	}
	for(int k=0;k<n;k++)
		for(int i=k+1;i<n;i++)
			for(int j=0;j<n;j++)
				A[i][j]+=A[k][j];
}

void print_result()
{
	for(int i=0;i<n;i++)
	{	for(int j=0;j<n;j++)
			cout<<A[i][j]<<" ";
		cout<<endl;

	}
}

void pthread_static()
{
    //初始化信号量
    sem_init(&sem_leader,0,0);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_init(&sem_Division[i],0,0);
        sem_init(&sem_Elimination[i],0,0);
    }
    //创建线程
    pthread_t handles[worker_count];
    threadParam_t param[worker_count];
    for(int t_id=0;t_id<worker_count;t_id++)
    {
        param[t_id].t_id=t_id;
        pthread_create(&handles[t_id],NULL,threadFunc,(void*)(param+t_id));
    }
    for(int t_id=0;t_id<worker_count;t_id++)
        pthread_join(handles[t_id],NULL);
    //销毁所有信号量
    sem_destroy(&sem_leader);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_destroy(&sem_Division[i]);
        sem_destroy(&sem_Elimination[i]);
    }
}



void pthread_static_sse()
{
    //初始化信号量
    sem_init(&sem_leader,0,0);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_init(&sem_Division[i],0,0);
        sem_init(&sem_Elimination[i],0,0);
    }
    //创建线程
    pthread_t handles[worker_count];
    threadParam_t param[worker_count];
    for(int t_id=0;t_id<worker_count;t_id++)
    {
        param[t_id].t_id=t_id;
        pthread_create(&handles[t_id],NULL,threadFunc1,(void*)(param+t_id));
    }
    for(int t_id=0;t_id<worker_count;t_id++)
        pthread_join(handles[t_id],NULL);
    //销毁所有信号量
    sem_destroy(&sem_leader);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_destroy(&sem_Division[i]);
        sem_destroy(&sem_Elimination[i]);
    }
}

void pthread_static_avx()
{
    //初始化信号量
    sem_init(&sem_leader,0,0);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_init(&sem_Division[i],0,0);
        sem_init(&sem_Elimination[i],0,0);
    }
    //创建线程
    pthread_t handles[worker_count];
    threadParam_t param[worker_count];
    for(int t_id=0;t_id<worker_count;t_id++)
    {
        param[t_id].t_id=t_id;
        pthread_create(&handles[t_id],NULL,threadFunc2,(void*)(param+t_id));
    }
    for(int t_id=0;t_id<worker_count;t_id++)
        pthread_join(handles[t_id],NULL);
    //销毁所有信号量
    sem_destroy(&sem_leader);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_destroy(&sem_Division[i]);
        sem_destroy(&sem_Elimination[i]);
    }
}

void pthread_static_avx512()
{
    //初始化信号量
    sem_init(&sem_leader,0,0);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_init(&sem_Division[i],0,0);
        sem_init(&sem_Elimination[i],0,0);
    }
    //创建线程
    pthread_t handles[worker_count];
    threadParam_t param[worker_count];
    for(int t_id=0;t_id<worker_count;t_id++)
    {
        param[t_id].t_id=t_id;
        pthread_create(&handles[t_id],NULL,threadFunc3,(void*)(param+t_id));
    }
    for(int t_id=0;t_id<worker_count;t_id++)
        pthread_join(handles[t_id],NULL);
    //销毁所有信号量
    sem_destroy(&sem_leader);
    for(int i=0;i<worker_count-1;i++)
    {
        sem_destroy(&sem_Division[i]);
        sem_destroy(&sem_Elimination[i]);
    }
}

//串行
void normal()
{
    for(int k=0;k<n;k++)
	{
		for(int j=k+1;j<n;j++)
			A[k][j]=A[k][j]/A[k][k];
		A[k][k]=1.0;
		for(int i=k+1;i<n;i++)
		{
			for(int j=k+1;j<n;j++)
				A[i][j]=A[i][j]-A[i][k]*A[k][j];
			A[i][k]=0;
		}
	}
}

int main()
{
    int step=10;
    for(;n<=2000;n+=step)
    {
    double time_0=0.0,time_2=0.0,time_2_sse=0.0,time_2_avx=0.0,time_2_avx512=0.0;
    //cin>>n;  //矩阵规模
	for(int i=0;i<n;i++)
		A = new float*[n];
    for(int i=0;i<n;i++)
		A[i] = new float[n];
    
    m_reset();
    gettimeofday(&val,NULL);
    for(int i=0;i<10;i++)
        normal();
    gettimeofday(&newval,NULL);
    time_0+=(newval.tv_sec - val.tv_sec) + (double)(newval.tv_usec - val.tv_usec) / 1000000.0;    
        
    m_reset();
    gettimeofday(&val,NULL);
    for(int i=0;i<10;i++)
        pthread_static();
    gettimeofday(&newval,NULL);
    time_2+=(newval.tv_sec - val.tv_sec) + (double)(newval.tv_usec - val.tv_usec) / 1000000.0;
    //print_result();
    
        
    m_reset();
    gettimeofday(&val,NULL);
    for(int i=0;i<10;i++)
        pthread_static_sse();
    gettimeofday(&newval,NULL);
    time_2_sse+=(newval.tv_sec - val.tv_sec) + (double)(newval.tv_usec - val.tv_usec) / 1000000.0;
    //print_result();
        
//     m_reset();
//     gettimeofday(&val,NULL);
//     for(int i=0;i<10;i++)
//         pthread_static_avx();
//     gettimeofday(&newval,NULL);
//     time_2_avx+=(newval.tv_sec - val.tv_sec) + (double)(newval.tv_usec - val.tv_usec) / 1000000.0;
        
//     m_reset();
//     gettimeofday(&val,NULL);
//     for(int i=0;i<10;i++)
//         pthread_static_avx512();
//     gettimeofday(&newval,NULL);
//     time_2_avx512+=(newval.tv_sec - val.tv_sec) + (double)(newval.tv_usec - val.tv_usec) / 1000000.0;

    cout<<"n:"<<n<<endl;
    cout<<"t:"<<time_0<<endl;
    cout<<"time:"<<time_2<<endl;
    cout<<"time_sse:"<<time_2_sse<<endl;
//     cout<<"time_avx:"<<time_2_avx<<endl;
//     cout<<"time_avx512:"<<time_2_avx512<<endl;
    for(int i=0;i<n;i++)
        delete A[i];
    delete A;
    if(n==100){step=100;}
    if(n==1000){step=1000;}
    }
    return 0;
}
